...
const ClientPortal: React.FC<ClientPortalProps> = ({ clientId }) => {
  ...
  const steps = [
    ...
    { label: 'Análisis de Impuestos (IVA)', status: 'in_progress', date: 'Hoy' },
    { label: 'Declaración F29 / Pago', status: 'pending', date: '20 May' }
  ];
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-3">
               <span className="text-[10px] font-black bg-blue-600 text-white px-2.5 py-1 rounded-lg uppercase tracking-widest shadow-lg shadow-blue-500/20">Portal Clientes</span>
               <span className="text-[10px] font-black bg-emerald-100 text-emerald-600 px-2.5 py-1 rounded-lg uppercase tracking-widest">Cumplimiento: Al Día</span>
            </div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-1">¡Hola, {client.name}!</h1>
            <p className="text-slate-500 font-bold text-sm">Aquí puedes ver qué está pasando con tus impuestos este mes.</p>
          </div>
          <button className="bg-blue-600 text-white px-10 py-5 rounded-[1.5rem] font-black shadow-2xl shadow-blue-600/30 hover:scale-105 transition-all flex items-center gap-3 uppercase text-xs tracking-widest border-none cursor-pointer">
            <FileUp size={20} /> Cargar Documentos de Mayo
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-10 relative">
              {steps.map((step, idx) => (
                <div key={idx} className="flex items-center gap-8 group">
                  ...
                  <div className="flex-1 flex justify-between items-center bg-slate-50/50 p-5 rounded-2xl border border-transparent hover:border-slate-200 transition-all cursor-default">
                    <div>
                    ...
                    {step.status === 'in_progress' && (
                      <span className="text-[10px] font-black bg-blue-600 text-white px-3 py-1.5 rounded-full shadow-lg shadow-blue-500/20 uppercase tracking-widest">En Proceso por Auditor</span>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
           <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Próximo Vencimiento SII</h4>
              <p className="text-5xl font-black tracking-tighter mb-2">12 DÍAS</p>
              <div className="bg-white/10 px-6 py-2 rounded-full border border-white/10 text-[10px] font-black uppercase tracking-widest italic">Vence 20 de Mayo</div>
           </div>

           <div className="bg-amber-50 border border-amber-200 p-8 rounded-[2.5rem] relative overflow-hidden">
              <h3 className="font-black uppercase tracking-tight text-sm text-amber-900 mb-6 flex items-center gap-2">
